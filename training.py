...
model = cm.Model(f1=20, f2=10, d1=20, input_dim=1, num_classes=2) # TODO insert parameters
x_train_mask, x_test_mask = cm.convert_data(with_mask) # TODO replace with correct data-retrieving function
x_train_without, x_test_without = cm.convert_data(without_mask) # TODO replace with correct data-retrieving function
...
for epoch_cnt in range(10):
    ...
    for batch_cnt in range(0, len(x_train) // batch_size):
        ...
        acc = cm.accuracy(pred, pred_true)
...
# evaluate model
pred = model(x_test)
accuracy = cm.accuracy(pred, y_test)
